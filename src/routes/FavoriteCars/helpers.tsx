...
const NoteItem: FC<{ record: any; text: string }> = ({ record, text }) => {
  const [filled, setFilled] = useState(false);

  return (
    <div>
      <div style={{ fontWeight: '500', fontSize: '17px', cursor: 'pointer' }}>
        <Dropdown
            ...
            items: [
              {
                key: 'note1',
                label:
                  record.mileage === 358000 ? (
                    <TextArea
                      autoSize
                      value="Гаражного хранения. Владелец предложил комплект зимних колес. Готов присутствовать в ГИБДД при оформлении"
                    />
                  ) : (
                    <TextArea autoSize onChange={() => setFilled(true)} />
                  ),
                onClick: (e) => {
          ...
          placement="topCenter"
        >
          {filled ? <PlusCircleFilled /> : <PlusCircleOutlined />}
        </Dropdown>
      </div>
    </div>
  );
...
export const favColumns = (
...
): ColumnsType<ICar> => [
  {
    ...
    render: (text, record, i) => <NoteItem record={record} text={text} />,
    align: 'center',
    width: '8%',
  },
  {
